        ...
        )
{
    double dist = ((double)NMEA_EARTHRADIUS_M) * acos(
        sin(to_pos->lat_r) * sin(from_pos->lat_r) +
        cos(to_pos->lat_r) * cos(from_pos->lat_r) * cos(to_pos->lon_r - from_pos->lon_r)
        );
    return dist;
}
        ...
        )
{
    ...
    if ((from_pos->lat_r == to_pos->lat_r) && (from_pos->lon_r == to_pos->lon_r))
    { /* Identical points */
        if ( from_azimuth != 0 )
            *from_azimuth = 0;
        if ( to_azimuth != 0 )
    ...
    )
{
    ...
    end_pos->lat_r=asin(sin(p1.lat_r)*cos(distance)+cos(p1.lat_r)*sin(distance)*cos(azimuth));
    end_pos->lon_r=p1.lon_r+atan2(sin(azimuth)*sin(distance)*cos(p1.lat_r),cos(distance)-sin(p1.lat_r)*sin(end_pos->lat_r));
    ...
    )
{
    ...
    { /* No move */
        *end_pos = *start_pos;
        ...
        // HBL return ! (NMEA_POSIX(isnan)(end_pos->lat) || NMEA_POSIX(isnan)(end_pos->lon));
        return end_pos->lat_r;
    } /* No move */
    ...
    /* Result */
    end_pos->lon_r = start_pos->lon_r + L;
    end_pos->lat_r = phi2;
    if ( end_azimuth != 0 )
    {
        *end_azimuth = atan2(
