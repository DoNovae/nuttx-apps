 ...
 * -------------------
 */
static const char * UI_AP_COMPASS = "Compass";
static const char * UI_AP_GPS = "GPS";
static const char * UI_AP_WIND = "Wind";
static const char * UI_AP_TRUE_WIND = "True Wind";
static const char * UI_AP_NAV = "Nav";
static const char * UI_AP_BLANK = " ";

//static const char* AP_MODE_COMPASS = "MODE (Compass)";
#define AP_MODE_COMPASS "MODE (Compass)"
static const char* AP_MODE_GPS = "MODE (GPS)";
static const char* AP_MODE_WIND = "MODE (Wind)";
static const char* AP_MODE_WIND_TRUE = "MODE (True Wind)";
static const char* AP_MODE_NAV = "MODE (Nav)";
...
/*
 * autopilot_btnm_map: text labels for autopilot_btnm
 */
...
/*
 * =============================
 * event_handler_ap_stats
 * -----------------------------
 * Associated to lv_autopilot_list_stats screen
 * Close lv_autopilot_list_stats screen
 * -----------------------------
 */
void event_handler_ap_stats(lv_event_t *e)
{
	lv_event_code_t code = lv_event_get_code(e);
	//LOG_D("event_handler_ap_stats: %s",code);
	//lv_indev_wait_release(lv_indev_get_act());

	if (code == LV_EVENT_CLICKED) {
		LOG_D("event_handler_ap_stats\n");
...
/*
 * =============================
 * lv_autopilot_list_stats
 * -----------------------------
 * Create screen autopilot_list_stats to display data values of
 * - Voltage
 * - Amp Hours
 * - Controller
 * Associated to event_handler_ap_stats
 * -----------------------------
 */
...
/*
 * =============================
 * event_handler_ap_mode
 * -----------------------------
 * Associated to lv_autopilot_list_modes screen
 * Close lv_autopilot_list_modes screen
 * -----------------------------
 */
void event_handler_ap_mode(lv_event_t *e)
{
	lv_event_code_t code;
	const char *mode;
	code = lv_event_get_code(e);
	mode= (const char *)lv_event_get_user_data(e);

	//lv_indev_wait_release(lv_indev_get_act());
	LOG_D("event_handler_ap_mode: code(%d)",code);
	LOG_D("event_handler_ap_mode: %s",mode);

	if (code == LV_EVENT_CLICKED)
	{
		LOG_D("event_handler_ap_mode: LV_EVENT_CLICKED");
		if (mode != UI_AP_BLANK) {
...
/*
 * =============================
 * lv_autopilot_list_modes
 * -----------------------------
 * Create list associate with event_handler_ap_mode
 * 	- Compas
 * 	- GPS
 * 	- Wind
 * 	- Wind true mode
...
//	lv_style_set_bg_color(&style_blue,lv_palette_main(LV_PALETTE_CYAN));
//	lv_obj_add_style(autopilot_list_modes,&style_blue, LV_PART_SCROLLBAR);
//
//	lv_obj_set_style_size(autopilot_list_modes,40,LV_PART_SCROLLBAR);
	//lv_obj_set_scroll_dir(autopilot_list_modes,LV_DIR_TOP);
	...
	btn = lv_list_add_btn(autopilot_list_modes, NULL, "TEST1");
	lv_obj_add_event_cb(btn, event_handler_ap_mode, LV_EVENT_CLICKED, (void *)"TEST1");

	btn = lv_list_add_btn(autopilot_list_modes, NULL, "TEST2");
	lv_obj_add_event_cb(btn, event_handler_ap_mode, LV_EVENT_CLICKED, (void *)"TEST2");

	btn = lv_list_add_btn(autopilot_list_modes, NULL, "TEST3");
	lv_obj_add_event_cb(btn, event_handler_ap_mode, LV_EVENT_CLICKED, (void *)"TEST3");
	btn = lv_list_add_btn(autopilot_list_modes, NULL, "TEST4");
	lv_obj_add_event_cb(btn, event_handler_ap_mode, LV_EVENT_CLICKED, (void *)"TEST4");
}
 ...
 * autopilot_event_cb
 * -----------------------------
 * Handler for autopilot_btnm
 * -----------------------------
 */
void autopilot_event_cb(lv_event_t *e)
{
	lv_event_code_t code = lv_event_get_code(e);
	lv_obj_t *obj = lv_event_get_target(e);

	LOG_D("autopilot_event_cb: code(%d)",code);
...
/*
 * =============================
 * lv_autopilot_display
 * -----------------------------
 * Create screns:
 *   - autopilot_led: led indicator
 *   - heading_l, command_l: data values
 *   - autopilot_btnm: buttons
 *   - lv_autopilot_list_modes: selection of modes
 *   - lv_autopilot_list_stats: display measurements
 *  -----------------------------
 */
void lv_autopilot_display(lv_obj_t *parent)
{
	...
	lv_led_set_brightness(autopilot_led, 150);
	lv_led_off(autopilot_led);
	 ...
	 * Width in the row of Mode(id 6)/Eye(id 7)
	 * 1 means 2/3, 1/3
	 */
	lv_btnmatrix_set_btn_width(autopilot_btnm, 6, 3);
	lv_obj_add_event_cb(autopilot_btnm, autopilot_event_cb, LV_EVENT_PRESSED, NULL);
